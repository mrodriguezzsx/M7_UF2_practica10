...
def densitatKM2():
    # Guardem les dades de l'excel en una variable
    dades = pd.read_csv("activitat_b.csv", usecols=['City', 'Area KM2'])
    ...
    plt.pie(dades["Area KM2"], labels=dades['City'], autopct='%1.1f%%')
    plt.legend(loc='upper right', bbox_to_anchor=(1.3, 1.1))
    return plt.show()

def densitatM2():
    # Guardem les dades de l'excel en una variable
    dades = pd.read_csv("activitat_b.csv", usecols=['City', 'Area   M2'])
    ...
    plt.pie(dades["Area   M2"], labels=dades['City'], autopct='%1.1f%%')
    plt.legend(loc='upper right', bbox_to_anchor=(1.3, 1.1))
    return plt.show()
